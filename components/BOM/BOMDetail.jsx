...
function BOMDetail({ route }) {
  ...
  const handleSave = async () => {
    try {
      ...
      Alert.alert('Success', 'BOM saved successfully', [{ text: 'OK' }]);
    } catch (error) {
    ...
    <View className="bg-primary h-full" style={{ flex: 1 }}>
          ...
          ListHeaderComponent={
            <>
              <Card style={styles.cardFirstContainer}>
                <Card.Title title={'ID: '+ bomDetail.id} titleStyle={styles.cardFirstTitle} />
                <Card.Content>
                  <Text style={styles.cardFirstInfo}>Role: Product Manager</Text>
                  <Text style={styles.cardFirstInfo}>Full name: {bomDetail.productManager?.fullName}</Text>
                  <Text style={styles.cardFirstInfo}>BOM Name: {bomDetail.bomname}</Text>
                </Card.Content>
              </Card>
              ...
              <Card style={styles.cardSecondContainer}>
                <Card.Content>
                  ...
                  <FormField
                    ...
                    handleChangeText={value => handleInputChange('dateCreation', value)}
                    otherStyles="mt-2"
                    edit={false}
                  />
                </Card.Content>
              ...
              <Card style={styles.cardFourthContainer}>
                <Card.Content>
                  <DataTable>
                    <DataTable.Header>
                      <DataTable.Title style={{ flex: 2 }}>Material</DataTable.Title>
                      <DataTable.Title style={{ flex: 1 }}>Unit</DataTable.Title>
                      <DataTable.Title style={{ flex: 1 }}>Price</DataTable.Title>
                      <DataTable.Title style={{ flex: 1 }}>Quantity</DataTable.Title>
                      <DataTable.Title style={{ flex: 0.5 }}></DataTable.Title>
                    </DataTable.Header>

                    {Array.isArray(bomDetail.materials) && bomDetail.materials.map((item, index) => (
                    <DataTable.Row key={index}>
                      <DataTable.Cell style={{ flex: 2 }}>{item.materialName}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 1 }}>{item.materialUnit}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 1 }}>{item.materialPrice}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 1 }}>{item.materialQuantity}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 0.5 }}>
                        <IconButton iconName="trash" onPress={() => handleDeleteMaterial(item.materialId)} />
                      </DataTable.Cell>
                    </DataTable.Row>
                    ))}
      ...
      <DataTable.Row>
        <DataTable.Cell style={{ flex: 2 }}>
          <TextInput placeholder="Material" value={newMaterialName} onChangeText={setNewMaterialName} />
        </DataTable.Cell>
        <DataTable.Cell style={{ flex: 1 }}>
          <Picker selectedValue={newMaterialUnit} onValueChange={setNewMaterialUnit} style={{ flex: 1 }}>
            <Picker.Item label="g" value="g" />
            <Picker.Item label="kg" value="kg" />
            <Picker.Item label="amount" value="amount" />
            <Picker.Item label="meter" value="meter" />
        ...
        <DataTable.Cell style={{ flex: 1 }}>
          <TextInput placeholder="Price" value={newMaterialPrice} onChangeText={setNewMaterialPrice} />
        </DataTable.Cell>
        <DataTable.Cell style={{ flex: 1 }}>
          <TextInput placeholder="Quantity" value={newMaterialQuantity} onChangeText={setNewMaterialQuantity} />
        </DataTable.Cell>
        <DataTable.Cell style={{ flex: 0.5 }}>
          <IconButton iconName="plus" onPress={handleAddMaterial} />
        </DataTable.Cell>
      </DataTable.Row>
    </DataTable>
  </Card.Content>
